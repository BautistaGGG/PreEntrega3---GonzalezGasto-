...
function ItemDetailContainer() {
  ...
  const { elementosEnCarrito, onAgregarAlCarrito } = useContext(ContextoCarrito)
    ...
    <main className="bg-[#030203] p-4">
      {isLoading ? <SpinnerLoader/> : 
        <>
          {
            historial.length > 2 ? 
            <NavLink to="/" className="inline-flex items-center self-baseline gap-2"> 
              <FontAwesomeIcon icon={faArrowLeft} style={{color: "#b05b30"}}/>
              <p className="text-white">Ir al inicio </p>
            </NavLink> : null
          }
          <li className="block m-auto max-w-[275px] list-none text-center rounded-2xl">
            ...
            <div className="px-2 bg-zinc-950 text-white">
              ...
              {elementosEnCarrito(APIdata.id) === 0 ? <p className="my-2">Stock disponible: {APIdata.stock}</p>: null}
              ...
              <button onClick={() => onAgregarAlCarrito(APIdata.id)} className="bg-zinc-950 border border-[#b05b30] rounded-md p-2 mb-4 hover:bg-[#b05b30]">
